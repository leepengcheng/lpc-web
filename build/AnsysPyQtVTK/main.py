...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def showCoordinate(self,isshow,sysname="world_sys"):
        if isshow:
            ...
            actors=[]
            axis_actor=createAxisActor(*params) #创建坐标系
    ...
    def loadImportData(self,dtype):
        ...
        if "model" in dtype and not "model" in vars(self):
            self.model = Model(self.points, self.elements, self.results, self.boundary)
        if "section" in dtype and not "section" in vars(self):
