...
proc main {} {\
	...
	wm geometry .dynamicpressure 360x500+$x+$y
	...
	grid $label_dens -row 7 -column 0 -sticky w -padx 2m 
	grid $entry_dens -row 7 -column 1 -sticky w -padx 2m 
...
proc applyUnderLoad {} \
{
	...
	foreach name [array names elements] {
		...
		foreach node $elements($name) {
			...
			set min_pressure [expr 0.5*$dens*$min_height*$min_height*$factor]
			set max_pressure [expr 0.5*$dens*$max_height*$max_height*$factor]
			##test expression
...
proc applySideLoad {} \
{
	...
	foreach name [array names elements] {
		...
		foreach node $elements($name) {
			...
			# set pressure [expr 0.5*$dens*$absheight*$absheight*$factor]
			...
			##calculate the pressure
			set min_pressure [expr 0.5*$dens*$min_height*$min_height]
			set max_pressure [expr 0.5*$dens*$max_height*$max_height]
